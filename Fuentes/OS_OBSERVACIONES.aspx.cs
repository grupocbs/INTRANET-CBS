...
public partial class OS_OBSERVACIONES : System.Web.UI.Page
{
...
cmb_estados.Items.Add(new ListEditItem("TODOS", "TODOS"));
cmb_estados.Items.Add(new ListEditItem("FINALIZADOS", "F"));
cmb_estados.Items.Add(new ListEditItem("PENDIENTES", "P"));
cmb_estados.SelectedIndex=0; 
    ...
    private void Cargar()
    {
        string sql = "SELECT USR_OPOBOJ_USERNM as SUP,USR_CLIOBJ_OBJDSC as OBJETIVO, USR_OPOBOJ_CODOBS as CODIGO,USR_OPOBOJ_FCHOBS as FECHA,USR_OPOBOJ_COORDN as COORDENADAS,CONVERT(VARCHAR(MAX), USR_OPOBOJ_OBSERV) as OBS,CONVERT(VARCHAR(MAX),USR_OPOBOJ_CORREC)  as RTA,USR_OPOBTP_DESCRP AS TIPO,e.USR_OPOBES_DESCRP as ESTADO, count(f.USR_OPOBCA_CPTITL) as FOTOS, o.USR_OP_FECALT";
...
if (cmb_estados.SelectedItem.Text != "TODOS")
        {
            sql += " and USR_OPOBOJ_ESTADO='" + cmb_estados.SelectedItem.Value.ToString() + "'";
        }
        sql += " GROUP BY USR_OPOBOJ_USERNM, USR_CLIOBJ_OBJDSC, USR_OPOBOJ_CODOBS,USR_OPOBOJ_FCHOBS,USR_OPOBOJ_COORDN,CONVERT(VARCHAR(MAX), USR_OPOBOJ_OBSERV),CONVERT(VARCHAR(MAX),USR_OPOBOJ_CORREC),	USR_OPOBTP_DESCRP,	e.USR_OPOBES_DESCRP, o.USR_OP_FECALT";
        sql += " ORDER BY o.USR_OP_FECALT DESC";
        DataTable dt = Interfaz.EjecutarConsultaBD("CBS", sql);

panel1.GroupingText="Observaciones Cargadas:" + dt.Rows.Count;
